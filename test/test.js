...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof dlogspace, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function fills a one-dimensional ndarray (endpoint=true)', function test( t ) {
	...
	strt = scalar2ndarray( 0.0, {
		'dtype': 'float64'
	});
	stp = scalar2ndarray( 3.0, {
		'dtype': 'float64'
	});
	...
	actual = dlogspace( [ x, base, strt, stp, endpoint ] );
	t.strictEqual( actual, x, 'returns expected value' );
	t.strictEqual( isEqualDataType( getDType( actual ), 'float64' ), true, 'returns expected value' );
	t.deepEqual( getData( actual ), new Float64Array( [ 1.0, 10.0, 100.0, 1000.0 ] ), 'returns expected value' );
...
tape( 'the function fills a one-dimensional ndarray (endpoint=false)', function test( t ) {
	...
	x = ndarray( 'float64', new Float64Array( [ 0.0, 0.0 ] ), [ 2 ], [ 1 ], 0, 'row-major' );
	base = scalar2ndarray( 2.0, {
		'dtype': 'float64'
	});
	strt = scalar2ndarray( 1.0, {
		'dtype': 'float64'
	});
	stp = scalar2ndarray( 3.0, {
		'dtype': 'float64'
	...
	t.strictEqual( actual, x, 'returns expected value' );
	t.strictEqual( isEqualDataType( getDType( actual ), 'float64' ), true, 'returns expected value' );
	t.deepEqual( getData( actual ), new Float64Array( [ 2.0, 4.0 ] ), 'returns expected value' );
